...
pub fn process(input: &str) -> miette::Result<String> {
    let (_, map) = parse(Span::new(input)).map_err(|e| miette!("parse failed {}", e))?;
...
fn search_trail(map: &HashMap<IVec2, u32>, position: &IVec2) -> usize {
            ...
            .filter(|(new_location, starting_location)| {
                !new_locations.contains(new_location)
                    && map.get(new_location).is_some_and(|h| {
                        let current_height = map.get(starting_location).unwrap();
