        ...
        .flat_map(|chunk| {
            chunk
                ...
                .flat_map(|sats| {
                    ...
                    let adiff = diff.clone();
                    let ax_bound = x_bound.clone();
                    let ay_bound = y_bound.clone();

                    let first_results = successors(
                        Some(sats[0].0),
                        Box::new(move |pos: &IVec2| {
                            let new_pos = pos + adiff;
                            if ax_bound.contains(&pos.x)
                                && ay_bound.contains(&pos.y)
                            {
                                Some(new_pos)
                            } else {
                                None
                    ...
                    let bdiff = diff.clone();
                    let bx_bound = x_bound.clone();
                    let by_bound = y_bound.clone();
                    let second_results = successors(
                        Some(sats[1].0),
                        Box::new(move |pos: &IVec2| {
                            let new_pos = pos - bdiff;
                            if bx_bound.contains(&pos.x)
                                && by_bound.contains(&pos.y)
                            {
                                Some(new_pos)
                            } else {
                                None
